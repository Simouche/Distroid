...
class ClientFields @Inject constructor() {
...
class ClientErrorFields @Inject constructor() {
    var code: Int? = null
    var name: Int? = null
...
class ClientForm @Inject constructor(
    val fields: ClientFields,
    val errors: ClientErrorFields
) : BaseObservable() {
    ...
    fun isCodeValid(setMessage: Boolean): Boolean {
        ...
        return if (code != null && code.length >= 5) {
            errors.code = null
    ...
    @Bindable
    fun getProvidererror(): Int? = errors.provider
